...
import { Building2, AlertTriangle, CheckCircle, Loader2, RefreshCw } from 'lucide-react';
...
export function DatabaseGate({ children }: DatabaseGateProps) {
  ...
  const handleRetry = () => {
    ...
    checkDatabaseConnection()
      .then((ok) => {
        setStatus(ok ? 'connected' : 'failed');
        if (!ok) setErrorDetail('The Supabase project is reachable but the database did not respond as expected.');
      })
      .catch((err: any) => {
        setStatus('failed');
        setErrorDetail(err?.message || 'Unknown connection error');
          ...
          <div className="glass-card p-8 border border-red-500/20">
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-gray-500 mt-3">
                Make sure you have run the schema from{' '}
                <code className="text-luxury-gold-400 bg-luxury-gold-500/10 px-1 rounded">supabase/schema.sql</code>{' '}
