...
export function BuyingSignalCard({ signal, index = 0 }: BuyingSignalCardProps) {
  const Icon = SIGNAL_ICONS[signal.type] || Newspaper;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        IMPACT_GLOWS[signal.impact]
      )}
    >
      <div className="flex items-start gap-3">
        <div className={cn(
          'w-10 h-10 rounded-xl flex items-center justify-center shrink-0',
          signal.impact === 'high' ? 'bg-emerald-500/15' :
          signal.impact === 'medium' ? 'bg-amber-500/15' : 'bg-gray-500/15'
        )}>
          <Icon className={cn(
            'w-5 h-5',
            signal.impact === 'high' ? 'text-emerald-400' :
            signal.impact === 'medium' ? 'text-amber-400' : 'text-gray-400'
          )} />
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={cn('inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium border', IMPACT_COLORS[signal.impact])}>
              {signal.impact}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between text-[10px] text-gray-500">
            <div className="flex items-center gap-2">
              <span className="text-luxury-gold-400 font-medium">{signal.source}</span>
              <span>·</span>
            ...
            <div className="flex items-center gap-1">
              <div className="flex items-center gap-1">
                <span className={cn(
                  ...
                  signal.relevanceScore >= 80 ? 'bg-emerald-400' :
                  signal.relevanceScore >= 60 ? 'bg-amber-400' : 'bg-gray-400'
                )} />
                <span className="text-gray-600">Rel: {signal.relevanceScore}%</span>
              </div>
              {signal.url && (
                <a href={signal.url} target="_blank" rel="noopener noreferrer" className="p-1 hover:bg-white/5 rounded text-gray-500 hover:text-white transition-colors">
